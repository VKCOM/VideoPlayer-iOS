...
class InplaceCustomControls: UIView, PlayerControlsViewProtocol {
    ...
    var controlMask: ControlMask? {
        didSet {
            ...
            if mask.hasControl(.loading) {
                ...
                if let pause = mask.getControl(.pause), sparked, case let .pause(paused) = pause {
            ...
            if mask.hasControl(.live) {
                if sparked, let rewindValues = mask.liveRewindValues, rewindValues.available > 0 {
                    timelineView.update(
                        progress: rewindValues.available - rewindValues.behind,
                ...
                if let timelineValues = mask.timelineValues, timelineValues.duration > 0 {
                    var animated = false
                    if let pause = mask.getControl(.pause), case let .pause(paused) = pause {
                        animated = !paused
                    }
                    timelineView.update(
                        progress: timelineValues.currentTime,
            ...
            if let sound = mask.getControl(.sound), sparked, case let .sound(enabled, restricted) = sound, mask.screencastState == nil {
    ...
    func handlePlayerTick() {
        guard let mask = controlMask, !mask.hasControl(.live), !timelineView.isHidden else {
            return
        }
    ...
    func timelineValueChanged() {
        ...
        guard let controlsDelegate else {
            return
        }

        // в случае live это будет отставание от реального времени
    ...
    lazy var settingsButton: UIButton = {
        ...
        button.accessibilityIdentifier = "video_player.settings_menu_button"
        button.accessibilityLabel = "video_player_settings_button_accessibility_label".ovk_localized()
    ...
    lazy var fullscreenButton: UIButton = {
        let button = UIButton(frame: CGRect(x: 0, y: 0, width: 32, height: 32))
        button.accessibilityIdentifier = "video_player.fullscreen_button"
    ...
    func makeSubtitlesViewIfNeeded() {
        ...
        view.accessibilityIdentifier = "video_player.subtitles_view"
        subtitlesView = view
    }
    ...
    private lazy var screencastButton: ScreencastButton = {
        ...
        button.accessibilityIdentifier = "video_player.screencast_button"
        button.accessibilityLabel = "video_player_screencast_menu_accessibility_label".ovk_localized()
        button.accessibilityHint = "video_player_screencast_menu_accessibility_hint".ovk_localized()
        button.translatesAutoresizingMaskIntoConstraints = false
        button.heightAnchor.constraint(equalToConstant: 32).isActive = true
        button.widthAnchor.constraint(equalToConstant: 32).isActive = true
        return button
