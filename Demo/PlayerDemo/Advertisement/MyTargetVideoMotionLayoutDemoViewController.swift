...
class MyTargetVideoMotionLayoutDemoViewController: UIViewController {
    ...
    private var tapsCounter: TapsCounter?
...
//        viewResizer.activate()
        ...
        tapsCounter = TapsCounter(maxCount: Int(duration)) { [weak self] in
            guard let self else {
                return
            }

            cardsView.update(currentTime: Double(self.tapsCounter?.count ?? 0), duration: duration)
        }
