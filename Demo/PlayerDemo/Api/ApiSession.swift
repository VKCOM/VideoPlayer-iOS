...
class ApiSession: NSObject {
    ...
    @objc
    private(set) static var shared: ApiSession?
    ...
    @objc
    static func setup(clientId: String, secret: String) {
    ...
    @objc
    static func setup(sessionToken: String, isAnonymous: Bool = false) {
    ...
    @objc
    func fetch(videoIds: [String], completion: @escaping ([Video]?, NSError?) -> Void) {
    ...
    @objc
    func fetch(
        ownerVideos ownerId: String,
        count: UInt = 10,
        offset: UInt = 0,
        completion: @escaping ([Video]?, NSError?) -> Void
    ) {
        performRequest(
            to: VKApi("video.get"),
            params: [
    ...
    private func obtainToken(completion: ((ApiSessionError?) -> Void)?) {
    ...
    private func performRequest<ResponseType: Decodable>(
        ...
        requireAuth: Bool,
        completion: @escaping (Result<ResponseType, ApiSessionError>) -> Void
    ) {
        ...
        if requireAuth, !isValid {
            let requestBlock: () -> Void = { [weak self] in
                self?.performRequest(
                    to: method,
                    params: params,
        ...
        let dataTask = URLSession.shared.dataTask(with: request) { [weak self] data, _, error in
            ...
            do {
                ...
                case .anonymousTokenIsInvalid,
                     .anonymousTokenHasExpired:
